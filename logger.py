...
class PeerLogger:
    ...
    def unchoked(self, other_id):
        self.logger.info(f"Peer {self.peer_id} is unchoked by {other_id}.")
    def choked(self, other_id):
        self.logger.info(f"Peer {self.peer_id} is choked by {other_id}.")
    def receiving_have(self, other_id, piece_index):
        self.logger.info(f"Peer {self.peer_id} received the 'have' message from {other_id} for the piece {piece_index}.")
    def receiving_interested(self, other_id):
        self.logger.info(f"Peer {self.peer_id} received the 'interested' message from {other_id}.")
    def receiving_not_interested(self, other_id):
        self.logger.info(f"Peer {self.peer_id} received the 'not interested' message from {other_id}.")
    def downloading_piece(self, other_id, piece_index, number_of_pieces):
        self.logger.info(f"Peer {self.peer_id} has downloaded the piece {piece_index} from {other_id}. Now the number of pieces it has is {number_of_pieces}.")
